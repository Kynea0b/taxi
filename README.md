...
+ 概念モデルにおいて最初「時間帯」を「割増率」に紐付けていたが「時間帯は時刻を持っているレコードで扱った方が良さそう」、「アプリ全体として時間帯の定義が各種メーターごとに異なるということはなさそう」という理由で「レコード」が持つ「時刻」に紐付けるという修正をした(以下画像参考) 
...
### 設計の見直しポイント
「運賃メーター」「距離メーター」「低速走行時間メーター」という概念を取り入れてみる。このメーターはEntityクラスになるのではという気がしている。
...
ver2.0では「総低速走行時間」を取得するルールをうまく表現できなかったので、運賃計算は一旦置いといて、レコードを受け取ってから「距離メーター」と「低速走行時間メーター」に記録するところまでをスコープにしてコードを書いてみた  
...
これをもとに「距離メーター」と「低速走行時間メーター」を記録する仕組みをコーディングしながら整理した  
その結果「低速走行時間メーター」は「距離メーター」を参照する形にしたのでレコードを持たない形になった  

↓こちらがその概念モデル  
<img width="800" alt="スクリーンショット 2024-04-02 0 25 12" src="https://github.com/reo-fujimoto2653/taxi/assets/82273732/e0129a7d-998f-4bc1-888e-67e7a825ff42">

↓この概念モデルに各運賃計算の仕組みを加えて表現したのがこちら
<img width="1026" alt="スクリーンショット 2024-04-03 19 44 08" src="https://github.com/reo-fujimoto2653/taxi/assets/82273732/b91a521d-7a7e-4908-bb69-ef0960ef5ac7">

↓概念モデルを元にコーディングして整理したver3.0のクラス図
<img width="1068" alt="スクリーンショット 2024-04-02 0 24 19" src="https://github.com/reo-fujimoto2653/taxi/assets/82273732/333cff68-a28d-4506-a74f-031d474afc7d">

### 所感
ver1.0のモデルとver3.0のモデルとでは全く異なるものになっていったので、  
+ 理解が深まることでモデルは変化していく
+ 何度も作り直す作業になっていく  

ということがわかった  

またこの作業は実際にコーディングすることによって進めることができることを体験できた  
※コーディング試験なので問題になっている仕様は読めば分かるが、どういうデータをどういうルールで扱えば良いか（ルールのまとめ方）はモデリングとコーディングを繰り返さないと理解が難しいと感じた
